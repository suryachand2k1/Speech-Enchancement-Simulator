...
def create_data(noise_id, voice_dir, path_save_spectrogram, sample_rate, min_duration, frame_length, hop_length_frame, hop_length_frame_noise, nb_samples, n_fft, hop_length_fft):

    list_noise_files = getNoiseByID(noise_id)#[:100]
    list_voice_files = os.listdir(voice_dir)#[:100]
    ...
    noise = audio_files_to_numpy('', list_noise_files, sample_rate, frame_length, hop_length_frame_noise, min_duration)
...
#     librosa.output.write_wav(path_save_sound + 'noise_long.wav', noise_long[0, :], sample_rate)
    ...
    m_amp_db_voice = numpy_audio_to_matrix_spectrogram(prod_voice, dim_square_spec, n_fft, hop_length_fft, phase=False)
    m_amp_db_noise = numpy_audio_to_matrix_spectrogram(prod_noise, dim_square_spec, n_fft, hop_length_fft, phase=False)
    m_amp_db_noisy_voice = numpy_audio_to_matrix_spectrogram(prod_noisy_voice, dim_square_spec, n_fft, hop_length_fft, phase=False)
...
#     np.save(path_save_time_serie + 'noisy_voice_timeserie', prod_noisy_voice)
    print('process audio done! saving numpy file...')
    ...
    print('save done! go to train model!')
