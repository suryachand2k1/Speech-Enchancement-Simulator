...
from src.data_tools import scaled_in, scaled_ou
...
def training(path_save_spectrogram, weights_path, name_model, training_from_scratch, epochs, batch_size):
    ...
    checkpoint = ModelCheckpoint(weights_path+'/model.h5', verbose=1, monitor='val_loss',save_best_only=True, mode='auto')

    generator_nn.summary()
    model_json = generator_nn.to_json()
    with open(weights_path + '/model.json','w') as f:
        f.write(model_json)
    #exit()
    #Training
